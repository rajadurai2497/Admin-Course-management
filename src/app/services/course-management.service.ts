...
export class CourseManagementService extends ApiService {
  ...
  getCourseChapters(courseMasterId, chapterId?, slideId?): Promise<any> {
    let apiUrl = ROUTE_CONFIG.CourseManagementURL + `/Admin/GetCourseGetChapterGetSlide?CourseMasterId=` + courseMasterId;
    if (chapterId) {
      apiUrl += `&ChapterId=` + chapterId;
    }
    if (slideId) {
      apiUrl += `&SlideId=` + slideId;
    }
    return this.doGet(apiUrl, true);
  }
